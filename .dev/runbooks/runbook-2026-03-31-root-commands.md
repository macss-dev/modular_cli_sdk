...
- [ ] Step 1: Add `command<I, O>()` to `ModularCli`
  - [ ] Write failing test(s) with documentation — root command registration and execution via `ModularCli.command()`
  - [ ] 🔍 REVIEW GATE
  - [ ] Incorporate feedback
  - [ ] Implement minimum code to pass — add method + 3 imports to `modular_cli.dart`
  - [ ] Refactor if needed
  - [ ] `git add . && git commit -m "step 1: root command registration on ModularCli"`

- [ ] Step 2: Root command with `--json` output
  - [ ] Write failing test(s) with documentation — root command produces valid JSON when `--json` is passed
  - [ ] 🔍 REVIEW GATE
  - [ ] Incorporate feedback
  - [ ] Implementation should pass with Step 1's code (lifecycle already handles `--json`)
  - [ ] Refactor if needed
  - [ ] `git add . && git commit -m "step 2: root command --json output"`

- [ ] Step 3: Root command coexistence with modules
  - [ ] Write failing test(s) with documentation — root commands and module commands both resolve correctly in the same CLI
  - [ ] 🔍 REVIEW GATE
  - [ ] Incorporate feedback
  - [ ] Implementation should pass with Step 1's code (cli_router handles dispatch)
  - [ ] Refactor if needed
  - [ ] `git add . && git commit -m "step 3: root command coexistence with modules"`

- [ ] Step 4: Root command validation failure
  - [ ] Write failing test(s) with documentation — root command with invalid input returns exit code 7
  - [ ] 🔍 REVIEW GATE
  - [ ] Incorporate feedback
  - [ ] Implementation should pass with Step 1's code (lifecycle already validates)
  - [ ] Refactor if needed
  - [ ] `git add . && git commit -m "step 4: root command validation failure"`

- [ ] Step 5: Example root command
  - [ ] Create `example/commands/version_command.dart` with `VersionInput`, `VersionOutput`, `VersionCommand`
  - [ ] Wire into `example/example.dart` via `cli.command('version', ...)`
  - [ ] Add test in `test/example_test.dart` for `['version']` args
  - [ ] Verify: `dart run example/example.dart version` and `dart run example/example.dart version --json`
  - [ ] `git add . && git commit -m "step 5: example root version command"`

- [ ] Step 6: Documentation + version bump
  - [ ] Update `README.md` — add "Root commands" section with usage example
  - [ ] Update `AGENTS.md` — add root command to architecture and patterns sections
  - [ ] Update `CHANGELOG.md` — add v0.2.0 entry
  - [ ] Bump `pubspec.yaml` version to 0.2.0
  - [ ] `git add . && git commit -m "step 6: docs + changelog + version bump to 0.2.0"`

- [ ] Step 7: Final validation
  - [ ] `dart analyze` — zero issues
  - [ ] `dart test` — all tests green
  - [ ] `dart format --output=none --set-exit-if-changed .` — 0 changed
  - [ ] `dart pub publish --dry-run` — no blocking errors
  - [ ] `git add . && git commit -m "step 7: final validation"`
